...
########################################################################################################################
def run_stage_1():
    def process_page_for_ids(page, traceID2time):
    ...
    traceID2time = {}
    num_processed = 0
    num_hits, num_total = process_page_for_ids(page, traceID2time)
    ...
    while num_hits > 0:
        ...
        print('Processed: %d/%d (%d%%) | %d traces' % (num_processed, num_total,
                                                       num_processed * 100 / num_total, len(traceID2time)))
        sid = page['_scroll_id']
...
def run_stage_2(experiment_id):
    def es_query(query, index=ELASTICSEARCH_INDEX, size=1000):
    ...
    for i, (dt, traces) in enumerate(buckets):
        ...
        for traceID in pbar:
            # Initialize the scroll
            results = es_query(query={'match': {'traceID': traceID}})
...
if __name__ == '__main__':
    ...
    # experiment_id = '202207131539_202207131610'

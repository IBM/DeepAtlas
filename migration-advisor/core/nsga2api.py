...
class NSGA2Recommender(object):
    @staticmethod
    def run(exp, pop_size=10, n_gen=100, critical_apis=()):
        movable_mask = np.asarray([msvc not in exp.constraints for msvc in exp.msvcs])

        algorithm = NSGA2(pop_size=pop_size, sampling=MPSampling(), crossover=MPCrossover(),
                          mutation=MPMutation(), eliminate_duplicates=MPDuplicateElimination())
        problem = MPProblem(exp, movable_mask, critical_apis)
        res = minimize(problem, algorithm, ('n_gen', n_gen), seed=2022, verbose=True)
...
class MPProblem(ElementwiseProblem):
    def __init__(self, experiment, movable_mask, critical_apis):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        _cost = self.experiment.cost_est.estimate(plan)
        _performance = self.experiment.performance_est.estimate(plan, self.critical_apis)
        _availability = self.experiment.availability_est.estimate(plan, self.critical_apis)

        out["F"] = [_cost, _availability, _performance]

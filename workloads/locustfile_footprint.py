...
GLOBAL_EXPERIMENT_DURATION = 60 * 10 * 10    # None = Run forever, 43200 = 12 hour
...
users_dummy_free = list(range(1100000, 1200000))
users_dummy_used = []
friendship = set()
...
class SocialNetworkUser(HttpUser):
    ...
    def check_cycle(self):
        if cycle == self.local_cycle:
            return
        with open('./footprint.txt', 'a') as o:
            o.write('%d,%f\n' % (cycle, time.time()))
        tasks = []
    ...
    compositions = [
        ...
        {login: 10, register: 10, follow: 10, unfollow: 1, readHomeTimeline: 10, readUserTimeline: 10, composePost: 10},

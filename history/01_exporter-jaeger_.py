...
########################################################################################################################
ELASTICSEARCH_HOST = 'http://c4130-110233.wisc.cloudlab.us:31590'
ELASTICSEARCH_INDEXES = ['jaeger-span-2022-08-02']
START = 1659414357
END = 1659419160
########################################################################################################################
...
def run_stage_1():
    def process_page_for_ids(index, page, traceID2time):
        for span in page['hits']['hits']:
            ...
            spanTime = span['_source']['startTime']
            if START <= spanTime * 1e-6 <= END:
                if traceID not in traceID2time:
                    traceID2time[traceID] = float('inf')
                traceID2time[traceID] = min(traceID2time[traceID], spanTime)
